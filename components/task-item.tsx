...
export function TaskItem({
  ...
  const title = useMemo(() => {
    const [subject, ...rest] = task.title.split("—");
  ...
  const RES = useMemo<ResourceBundle>(() => {
    ...
    return getResources(task.title);
  }, [task]);
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-[#111418] p-6">
      <div className="mb-4 flex items-start justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          {completed ? (
            <button
              onClick={unfinish}
              className="inline-flex items-center gap-1 rounded-full bg-emerald-500/10 px-3 py-1.5 text-emerald-300 text-xs ring-1 ring-emerald-500/30 hover:bg-emerald-500/20"
          ...
          <div className="rounded-lg border border-white/10 bg-[#0F1216] p-4">
            ...
            <ul className="space-y-2 text-sm">
              {(RES.videos.length ? RES.videos : [
                { label: "YouTube — videoaulas sobre o tema", url: `https://www.youtube.com/results?search_query=${encodeURIComponent(task.title + " FUVEST ENEM")}` },
              ]).map((v, i) => (
                <li key={i}>
                  <a
          ...
          <div className="rounded-lg border border-white/10 bg-[#0F1216] p-4">
            ...
            <ul className="grid sm:grid-cols-2 gap-2 text-sm">
              {(RES.materials.length ? RES.materials : [
                { label: "Khan Academy — pesquisa", url: `https://pt.khanacademy.org/search?page_search_query=${encodeURIComponent(task.title)}` },
              ]).map((m, i) => (
                <li key={i}>
                  <a
                    className="block rounded-md bg-[#0D1014] px-3 py-2 ring-1 ring-white/10 hover:bg-white/5 transition"
